...
exp_tpm <- fread("~/RScripts/genes_tpm_monomacro_processed.csv.gz")
meta_datos <- fread("~/RScripts/metadata_monomacro_filtered.csv.gz")
exp_raw <- fread("~/RScripts/genes_expected_count_DESeq2.csv.gz")
```
...
A continuación, vamos a obtener la red para M0 usando Esearch3D

```{r}
m0<- meta_S0 %>%
  filter(harmonized_cell_type == "macrophage")

colsmon <- c("id_col",intersect(m0$EpiRR, colnames(exp_tpm)))
monact <- exp_tpm[, ..colsmon]
